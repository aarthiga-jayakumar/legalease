...
# ------------------------------
# UI: Apple-like home + theme toggle
# ------------------------------
...
# Home screen: three large cards (Apple-like)
if st.session_state.page == "home":
    ...
    # Card function to navigate
    def card_html(icon, title, desc, action):
        btn = f"st.session_state.page='{action}'"
        html = f"""
    ...
    # capture clicks via anchors (simple)
...
if st.session_state.page == "qa":
    st.header("Ask Questions (QA) — get grounded answers from a document")
    st.write("Upload a doc in Analyze tab or paste text here as context.")
    question = st.text_input("Your question (clear & short)")
    ctx = st.text_area("Optional: paste the context here (overrides Analyze)", height=200)
    if st.button("Answer"):
        if ctx and ctx.strip():
            context = ctx
        else:
            # try to read last uploaded file in session (best-effort)
            if 'pdf_file' in locals() and pdf_file:
                context = extract_text_from_pdf_bytes(pdf_file.read())
            else:
                st.error("No context available. Paste text or upload in Analyze first.")
                st.stop()
...
if st.session_state.page == "compare":
    st.header("Compare Two Contracts — Upload or paste A and B")
    colA, colB = st.columns(2)
    with colA:
        fileA = st.file_uploader("Contract A (PDF)", type=["pdf"], key="compA")
        textA = st.text_area("Or paste Contract A text", height=120)
    with colB:
    ...
    if st.button("Run Comparison"):
        if textA.strip():
            ...
            A = extract_text_from_pdf_bytes(fileA.read())
            srcA = getattr(fileA,"name","A.pdf")
        else:
            st.error("Provide Contract A"); st.stop()
        if textB.strip():
            B = clean_text(textB)
            srcB = "manual"
        elif fileB:
            B = extract_text_from_pdf_bytes(fileB.read())
            srcB = getattr(fileB,"name","B.pdf")
        else:
            st.error("Provide Contract B"); st.stop()
